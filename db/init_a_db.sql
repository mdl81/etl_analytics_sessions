...
FROM '/docker-entrypoint-initdb.d/project_a_user_sessions.csv'
DELIMITER ','
CSV HEADER;

COPY pages(id, name, created_at)
FROM '/docker-entrypoint-initdb.d/project_a_pages.csv'
DELIMITER ','
CSV HEADER;

COPY events(id, user_id, event_name, page_id, created_at)
FROM '/docker-entrypoint-initdb.d/project_a_events.csv'
DELIMITER ','
CSV HEADER;